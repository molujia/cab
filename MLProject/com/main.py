...
def Mark(image, location, answer):
    ...
    # cv2.rectangle 靠确定对角线来画矩形,参数为
    # cv2.rectangle(img, pt1, pt2, color[, thickness[, lineType[, shift]]]) → None
    # 大多情况下写到线宽就可以
    # cv2.rectangle(img,(左上点的 x坐标,左上点的 y坐标),(右下点的 x坐标, 右下点的 y坐标), color=(0,0,255), 线宽=2)
    # 若 location的数据结构为 numpy的二维数组，每个元素的四个参数可能为[左上点的x,左上点的y,w,h]
    # 也可能为[左,上,右,下]

    result_image = image.copy()
    for i in range(0, len(location)):
        cv.rectangle(result_image, (location[i][0], location[i][1]),
                     (location[i][0] + location[i][2], location[i][1] + location[i][3]), (0, 0, 255), 2)
...
if __name__ == '__main__':
    ...
    # 导入图片，转化为 ndarray
    images_path = f'images' + os.sep + f'{picture_name}.jpg'
    ...
    # 显示表达式的位置信息
    for i in locations:
        cv.rectangle(image, (i[0], i[1]), (i[0] + i[2], i[1] + i[3]), (255, 0, 0))
    resized = cv.resize(image, (1000, 600))
    ...
    blank_path = f'images' + os.sep + f'blank.jpg'
    blank_image = cv.imread(blank_path)
    reverse_blank_image = cv.bitwise_not(blank_image)

    ans = []
    # 将所有分割后的字符暂存到一个文件中，做训练用
    for i in range(len(chara_sets)):
        # os.mkdir('expression')
        path = 'expression'+str(picture_name)+'/' + str(i)
        os.makedirs(path)
        for j in range(len(chara_sets[i])):
            cv.imwrite(f'{path}{os.sep}{picture_name}_{i}_{j}.png', chara_sets[i][j])
        # cv.imwrite(f'chars{os.sep}{picture_name}_{i}_blank.png', reverse_blank_image)
        tempres=pre_with_foldername.predict_with_folder(path)
        ans.append(tempres)
        # shutil.rmtree('expression')
    print(ans)
    print(len(ans))
    result = []
    for i in ans:
    ...
    cv.destroyAllWindows()
    # 最终结果
    # result_image = Mark(image, locations, answers)
    # cv.imshow('result', result_image)
    # cv.waitKey()
    # cv.destroyAllWindows()
